...
import type { Bolgeler } from "@/lib/supabase"

interface RegionalLandingProps {
  siteData: Bolgeler
}

export function RegionalLanding({ siteData }: RegionalLandingProps) {
  const [isMenuOpen, setIsMenuOpen] = useState(false)

  const navigationItems = siteData.navigation || []
  const heroButtons = siteData.hero_buttons || []
    ...
    <div className="bg-gray-900 text-white min-h-screen">
      ...
      <section className="pt-32 pb-20 bg-gradient-to-br from-gray-900 via-amber-900 to-gray-900">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-amber-500 to-gray-900 bg-clip-text text-transparent">
            {siteData.hero_title}
          </h1>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {stats.map((stat, index) => (
              <div key={`${stat.label}-${index}`} className="animate-slide-up" style={{ animationDelay: `${index * 0.1}s` }}>
      ...
      <section className="py-16 bg-gray-900">
        <div className="container mx-auto px-4">
          ...
          <ServiceCarousel districts={serviceAreas} />
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {advantages.map((advantage, index) => (
              <div key={`${advantage.title}-${index}`} className="backdrop-blur-md bg-gray-900/70 border border-gray-700 rounded-lg p-6">
                <div className="text-4xl text-amber-500 mb-4">
